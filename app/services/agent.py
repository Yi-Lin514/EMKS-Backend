...
AGENT_SYSTEM_PROMPT = (
    ...
    "3. **特別注意：當使用者問的是統計性、排名、熱門度、頻率、趨勢類問題**（例如「大家最常問什麼」、\n"
    "   「最熱門的文件」、「最近趨勢」等需要彙總分析的問題），如果你沒有對應的統計工具，\n"
    "   **必須直接拒答並說明沒有權限查詢這類統計**。不可以用 `search_knowledge_base` 或 `get_recent_documents` \n"
    "   的結果整理成「常見主題」「常見問題」等內容來代替統計回答 — 這會讓使用者誤以為看到的是真實統計，\n"
    "   屬於資訊誤導，比直接拒答更糟糕。\n"
